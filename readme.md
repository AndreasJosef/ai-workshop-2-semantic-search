...
3. **Search** — a small web UI: type a query, get the top 5 matches (snippet, score, source-article link for attribution), and flip a 768 / 3072 / Keyword toggle to see how the ranking changes.
4. **Compare** — a CLI that runs the same query across all three modes side by side, used to produce the [dimension comparison](./.scratch/wheel-of-time-search/dimension-comparison.md) and the [keyword-vs-semantic comparison](./.scratch/wheel-of-time-search/keyword-vs-semantic-comparison.md).
...
Schema: one `documents` table, one row per chunk, with `embedding_768 vector(768)` and `embedding_3072 vector(3072)` columns, two matching RPCs (`match_documents_768`, `match_documents_3072`), and a generated `content_tsv tsvector` column with a GIN index and a `match_documents_keyword` RPC for full-text search — see [ADR-0001](./docs/adr/0001-single-table-dual-embedding-columns.md) for why it's one table rather than two and [ADR-0004](./docs/adr/0004-postgres-full-text-keyword-search.md) for the keyword path.
...
Or compare all three modes for one or more queries from the CLI:
...
- [`.scratch/wheel-of-time-search/keyword-vs-semantic-comparison.md`](./.scratch/wheel-of-time-search/keyword-vs-semantic-comparison.md) — the keyword vs semantic write-up
